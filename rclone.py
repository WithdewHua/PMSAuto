...
def auto_rclone(src_path, dest_path):
    ...
    with instance_check.acquire(timeout=0):
        ...
        # 等待，以便rclone完全起起来
        logger.info('Wait %s seconds to full call rclone command: %s' % (check_after_start, cmd_rclone))
        ...
        while True:
            ...
            # 输出当前情况
            logger.info('Transfer Status - Upload: %s GiB, Avg upspeed: %s MiB/s, Transfered: %s, ETA: %s.' % (
