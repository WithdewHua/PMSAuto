...
MEDIA_INFO_CACHE = SQLiteCache(
    os.path.join(DATA_DIR, "media_info.sqlite3"),
    table_name="media_info",
    legacy_pickle_paths=[
        os.path.join(DATA_DIR, "media_info.cache"),
        os.path.join(DATA_DIR, "media_info.cache.bak"),
    ],
)
                        ...
                        if "up_done" not in tags:
                            ...
                            if query_flag:
                                ...
                                for (
                                    ...
                                    if (
                                        start_year is None or start_year <= int(year)
                            ...
                            # 如果是文件夹
                                ...
                                if not torrent_files:
                                    logger.error(
                                        f"Can not find files of {torrent.name}"
                                    )
                                    send_tg_msg(
                                        chat_id=TG_CHAT_ID,
