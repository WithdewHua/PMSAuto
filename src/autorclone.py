...
def auto_rclone(src_path, dest_path, files_from=None, action="copy"):
    ...
    with instance_check.acquire(timeout=0):
        ...
        cmd_rclone = f'rclone {action} "{src_path}" "{dest_path}"  --drive-chunk-size 512M --buffer-size 256M --rc --drive-server-side-across-configs -vv --log-file {rclone_log_file} --rc-addr {rc_addr}'
        if files_from:
        ...
        while True:
            ...
            while True:
                ...
                # 如果 eta 为 0, 且 transfers 不为 0, 则不再检查 switch 条件
                if (
                    response_json.get("eta", -1) == 0
                    and response_json.get("transfers", 0) != 0
                ):
                    logger.info(
